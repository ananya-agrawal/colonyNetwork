  ...
  async addSingleReputationUpdate(updateNumber, repCycle) {
    ...
    if (updateNumber.lt(this.nReputationsBeforeLatestLog)) {
      ...
      if (bnScore.isNeg()) {
        ...
        const innerUpdateNumber = updateNumber.sub(new BN(logEntry[5].toString())).sub(this.nReputationsBeforeLatestLog);
        ...
        if (
          ...
          (innerUpdateNumber.gte(nUpdates.divn(2)) && innerUpdateNumber.lt(nUpdates.divn(2).add(nChildUpdates)))
        ) {
          ...
          if (keyAlreadyExists) {
            ...
            const existingValueForActualUserSkillRep = new BN(valueForActualUserSkillRep.slice(2, 66), 16);
