...
function correctRecoveryModifier(functionDef) {
  ...
  const hasModifier = functionDef.modifiers.filter(mod => ["stoppable", "repair"].indexOf(mod.name) > -1).length > 0;
...
walkSync("./contracts/").forEach(contractName => {
  ...
  // Basically only ColonyNetworkMining.sol, ReputationMiningCycle.sol are
  if (
    [
  ...
  contract.subNodes.filter(child => child.type === "FunctionDefinition").forEach(functionDef => {
    if (!correctRecoveryModifier(functionDef)) {
      console.log("The contract", contractName, "contains a missing stoppable/repair modifier in function", functionDef.name, ".");
      process.exit(1);
    }
  });
});
