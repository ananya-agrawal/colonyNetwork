...
contract("ColonyNetworkMining", accounts => {
  ...
  async function accommodateChallengeAndInvalidateHash(test, client1, client2) {
    ...
    const repCycle = await ReputationMiningCycle.at(reputationMiningCycleAddress);
    let round2;
    let idx2;
    let toInvalidateIdx;
  ...
  afterEach(async () => {
    ...
    let nSubmittedHashes = await repCycle.nSubmittedHashes();
    nSubmittedHashes = nSubmittedHashes.toNumber();
    if (nSubmittedHashes > 0) {
      let nInvalidatedHashes = await repCycle.nInvalidatedHashes();
  ...
  describe("Basic Functionality - no client used", () => {
    ...
    it("should allow a new reputation hash to be submitted", async () => {
      ...
      const submitterAddress = await repCycle.submittedHashes("0x12345678", 10, 0);
  ...
  describe("Rewards and punishments of good and bad submissions", () => {
    it("should punish all stakers if they misbehave (and report a bad hash)", async () => {
      ...
      assert(userLock2[1].eq(new BN("1000000000000000000")));
  ...
  describe("Misbehaviour during dispute resolution", () => {
    ...
    it("should prevent a hash from advancing if it might still get an opponent", async function advancingTest() {
      ...
      repCycle = await ReputationMiningCycle.at(reputationMiningCycleAddress);
      await forwardTime(3600, this);
  ...
  describe("Intended ('happy path') behaviours", () => {
    it("should cope with many hashes being submitted and eliminated before a winner is assigned", async function manySubmissionTest() {
      ...
      while (nRemainingHashes > 1) {
        ...
        cycle += 1;
        const nInvalidatedHashes = await repCycle.nInvalidatedHashes(); // eslint-disable-line no-await-in-loop
    ...
    it("should keep reputation updates that occur during one update window for the next window", async () => {
      ...
      addr = await colonyNetwork.getReputationMiningCycle(false);
      let inactiveReputationMiningCycle = await ReputationMiningCycle.at(addr);
      ...
      const reputationMiningCycle = await ReputationMiningCycle.at(addr2);
      ...
      // the accepted hash.
      addr = await colonyNetwork.getReputationMiningCycle(false);
      inactiveReputationMiningCycle = await ReputationMiningCycle.at(addr);
  ...
  describe("when in repair mode", async () => {
    it("should be able to enter repair mode and exit repair mode", async () => {
      ...
      let isInRepairMode = await colonyNetwork.isStopped();
      assert(isInRepairMode, "Not in repare mode");

      await colonyNetwork.start();

      isInRepairMode = await colonyNetwork.isStopped();
    ...
    it("should be able to allow recovery role to call functions in recovery mode", async () => {
      const authority = await ColonyNetworkAuthority.new(colonyNetwork.address);

      const recoveryUser = OTHER_ACCOUNT;
      const recoveryRole = 0;
      await authority.setUserRole(recoveryUser, recoveryRole, true);
      await colonyNetwork.setAuthority(authority.address);
